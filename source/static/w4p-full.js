(function(){
  ...
  // === W4P-FULL LOAD GUARD ===
  if (window._w4p_full_injected) {
    console.log('[W4P-FULL] Clearing old instance - upgrading');
    if (window._w4p_full_timer) {
      clearInterval(window._w4p_full_timer);
      window._w4p_full_timer = null;
    }
    window._w4p_full_injected = false;
  }
  ...
  // === PUBLIC API ===
  window._w4p_full_getSnapshot = function() {
    try { return buildSnapshot(); } catch (e) { return null; }
  };

  window._w4p_full_status = function() {
    return {
      injected: true,
      posts: postCount,
      errors: errorCount,
  ...
  // === START ===
  window._w4p_full_timer = setInterval(tick, TICK_MS);
  window._w4p_full_injected = true;

  // Run first tick immediately
  tick();

  console.log(LOG_PREFIX + ' v1.0 loaded | hero=' + HERO_NAME + ' | API=' + API_URL + ' | tick=' + TICK_MS + 'ms');
  console.log(LOG_PREFIX + ' Status: window._w4p_full_status() | Manual snapshot: window._w4p_full_getSnapshot()');
})();
