"""
PLO5 6-MAX EXACT EQUITY ENGINE
================================
...
EPS         = 1e-9
MAX_PLAYERS = 6  # variant max; actual count determined from input
NUM_PLAYERS = 6  # will be overridden after parsing input
HOLE_CARDS = 5
...
def parse_variant(variant_key: str) -> dict:
    ...
    Must be called before any validation, parsing, or storage.
    Returns a rules dict with players, hole_cards, and line count bounds.
...
if __name__ == "__main__":
    ...
    hdr("PLO5 6-MAX EXACT EQUITY ENGINE", Fore.MAGENTA)
    ...
    # Flexible player count: accept 2+ hands (not just MAX_PLAYERS)
    # Detect board by card count: hand = HOLE_CARDS*2 chars, board = 6/8/10 chars
    ...
    # Ambiguity fallback: if no board found, last line might be board
    ...
    # Override NUM_PLAYERS with actual count
    NUM_PLAYERS = len(hand_indices)
    if NUM_PLAYERS < 2:
        err(f"Need at least 2 hands, got {NUM_PLAYERS}")
        sys.exit(1)
    if NUM_PLAYERS > MAX_PLAYERS:
        err(f"Too many hands ({NUM_PLAYERS}) for {MAX_PLAYERS}-max variant")
        sys.exit(1)
    ...
    if board_idx is not None:
        ...
        if board_ncards == 3:
            ...
            print(f"  Parsed: {'  '.join(flop_labels)}  {turn_labels[0]}  {river_labels[0]}")
