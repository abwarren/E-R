"""
PLO6 6MAX EXACT EQUITY ENGINE
Usage:
  python plo6-6max.py hands.txt
  python plo6-6max.py hands.txt --names playermapping.txt
"""
...
if __name__ == "__main__":
    ...
    hdr("PLO6 6MAX EXACT EQUITY ENGINE", Fore.MAGENTA)
    ...
    if not cli["input_file"]:
        err("Usage: python plo6-6max.py hands.txt [--names playermapping.txt]"); sys.exit(1)
    name_map = {}
    if cli["names_file"]:
    ...
    board = []
    if len(non_empty_lines) >= NUM_PLAYERS + 1:
        section("PARSING FLOP", Fore.CYAN)
        flop_cards, flop_labels = parse_board_line(non_empty_lines[NUM_PLAYERS], 3, "Flop")
        board.extend(flop_cards); all_known_cards.extend(flop_cards)
        field("Flop", non_empty_lines[NUM_PLAYERS])
        print(f"  Parsed: {'  '.join(flop_labels)}"); ok("Flop parsed")
    if len(non_empty_lines) >= NUM_PLAYERS + 2:
